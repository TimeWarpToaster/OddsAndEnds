    ...
    public static class L
    {
        ...
        public static bool logInit(string _logPath, bool _isFileLogging)
        {
            ...
            try
            {
                ...
                else if (!_isFileLogging) retValue = true;// prior versions had the bug (!isFileLogging vs (!_isFileLogging
                L.isFileLogging = _isFileLogging;
            }
            catch (Exception ex)
            {
                Console.WriteLine("L.logInit(lb) - Exception - Failed to initialize log UI!");
            }
            return retValue;
        }
        ...
        public static bool logInit(string _logPath, ListBox inLbLog, bool _isFileLogging)
        {
            ...
            try
            {
                ...
                {
                    L.err("L.logInit", "Input log was (" + (inLbLog == null ? "null" : "not null") + 
                        "), File path (" + (_logPath == null ? "null" : "not null") + 
                        "), isFileLogging (" + _isFileLogging + ").");
                }
            }
            catch (Exception ex)
            {
        ...
        public static bool logInit(string _logPath, RichTextBox inRtbLog, bool _isFileLogging)
        {
            ...
            {
                Console.WriteLine("L.logInit(rtb) - Exception - Failed to initialize log UI!");
            }
            return retValue;
        }
        ...
        public static void logWriter(string s)
        {
            ...
            {
                Console.WriteLine("L.logWriter - Failed to write log file!");
            }
        }
            ...
            try
            {
                if (s != null && s.Length > 0)
                {
                    if (L.lbLog != null)
                    {
                        ...
                        {
                            L.lbLog.Invoke(new Action(() => updateUI(s)));
                        }
                        else
                        {
                            L.lbLog.Items.Add(s);
                    ...
                    if (L.rtbLog != null)
                    {
                        ...
                        {
                            L.rtbLog.Invoke(new Action(() => updateUI(s)));
                        }
                        else
                        {
                            L.rtbLog.Text += s + "\n";
